 ...
 * =============================================================================
 */
package com.stone.demo.payment.core;
...
public class HttpClient {
     ...
     * @throws Exception
     */
    public int send(JSONObject data, boolean validateRemoteCert, String encoding) throws Exception {
        try {
            HttpURLConnection httpURLConnection = createConnection(validateRemoteCert, encoding);
     ...
     * @throws Exception
     */
    public int sendGet(boolean validateRemoteCert, String encoding) throws Exception {
        try {
            HttpURLConnection httpURLConnection = createConnectionGet(validateRemoteCert, encoding);
            if (null == httpURLConnection) {
     ...
     */
    private HttpURLConnection createConnection(boolean validateRemoteCert, String encoding) throws ProtocolException {
     ...
     * @throws ProtocolException
     */
    private HttpURLConnection createConnectionGet(boolean validateRemoteCert, String encoding) throws ProtocolException {
        HttpURLConnection httpURLConnection = null;
        try {
            httpURLConnection = (HttpURLConnection) url.openConnection();
        } catch (IOException e) {
     ...
     * @return
     */
    private String getRequestParamString(JSONObject requestParam, String encoding) {
        ...
        for (Map.Entry<String, Object> entry : requestParam.entrySet()) {
            try {
                sf.append(entry.getKey() + "=" + (null == entry.getValue() || "".equals(entry.getValue()) ? "" : URLEncoder.encode(String.valueOf(entry.getValue()), encoding)) + "&");
            } catch (UnsupportedEncodingException e) {
                LogUtil.writeErrorLog(e.getMessage(), e);
                return "";
            }
        }
        String reqstr = sf.substring(0, sf.length() - 1);
