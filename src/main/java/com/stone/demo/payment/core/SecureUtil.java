...
public class SecureUtil {
	 ...
	 * @return
	 */
	private static byte[] sha1(String data, String encoding) {
		try {
	 ...
	 * @return
	 */
	public static String encryptData(String datatring, String encoding,
			PublicKey key) {
		/** 使用公钥对密码加密 **/
		byte[] data = null;
		try {
			data = encryptData(key, datatring.getBytes(encoding));
	 ...
	 * @return 解密后的数据
	 */
	public static String decryptData(String datatring, String encoding,
			PrivateKey key) {
		byte[] data = null;
		try {
			data = SecureUtil.base64Decode(datatring.getBytes(encoding));
